...
# hadoop
RUN curl -s http://www.eu.apache.org/dist/hadoop/common/hadoop-2.5.1/hadoop-2.5.1-src.tar.gz | tar -xz -C /tmp/

# protoc -ohhh
RUN curl https://protobuf.googlecode.com/files/protobuf-2.5.0.tar.bz2 | bunzip2|tar -x -C /tmp
RUN cd /tmp/protobuf-2.5.1 && ./configure
RUN cd /tmp/protobuf-2.5.1 && make && make install
ENV LD_LIBRARY_PATH /usr/local/lib
ENV export LD_RUN_PATH /usr/local/lib

# build native libs
RUN cd /tmp/hadoop-2.5.1-src && mvn package -Pdist,native -DskipTests -Dtar
