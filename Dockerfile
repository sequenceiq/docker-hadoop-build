...
# hadoop
RUN curl -s http://www.eu.apache.org/dist/hadoop/common/hadoop-2.4.0/hadoop-2.4.0-src.tar.gz | tar -xz -C /tmp/
...
# build native libs
RUN cd /tmp/hadoop-2.4.0-src && mvn package -Pdist,native -DskipTests -Dtar 
